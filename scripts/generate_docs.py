...
.example { background: #0b1021; color: #d6e1ff; padding: 12px; border-radius: 4px; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; font-size: 0.9em; white-space: pre-wrap; }
...
<div class=\"operation\">\n  <h3 id=\"q-{item_key}\">{item.get('label', item_key)}</h3>\n  <p class=\"description\">{item.get('description','')}</p>\n  <div class=\"parameters\">{render_params(params)}</div>\n  <div class=\"example\">{example_code}</div>\n</div>\n"""
    ...
    for cat_key, cat in transition_defs.items():
        ...
        for item_key, item, ex in entries:
            ...
            example_code = escape(format_example(ex, '// Evo SDK example (requires keys/funding)'))
            t_html.append(
                f"""
...
<body>
  ...
  <script type=\"module\">import {{ EvoSDK }} from './dist/sdk.js'; window.EvoSDK = EvoSDK;</script>
</body>
</html>
"""
    return html
...
def format_example(code: str, header: str) -> str:
    ...
    header_line, *rest = formatted.split('\n') if '\n' in formatted else [formatted]
    const_line = 'const client = EvoSDK.testnetTrusted();'
    lines = [header, const_line]
    if header_line and header_line != const_line:
        lines.append(header_line)
    lines.extend(line for line in rest if line != const_line)
    return '\n'.join(lines).rstrip()
...
def main():
    ...
    (PUBLIC_DIR / 'docs_manifest.json').write_text(_json.dumps(manifest, indent=2), encoding='utf-8')
